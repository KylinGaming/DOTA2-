...
def player_code():
    ...
    with open("紫猫.txt", "r", encoding="utf-8") as fp:#这里改成你要解析的日志文件的名字
...
def hero_neme(j):
    ...
    nums=10
    nums=int(nums)
    findplay_hero = re.compile(r'"time":1,"type":"interval","unit":"CDOTA_Unit_Hero_(.*?)","slot":.*?')
...
def findkill_damage():
    ...
    damage_data_list = []
    kill_list = []
    with open("紫猫.txt", "r", encoding="utf-8") as fp:#将所有击杀信息与伤害信息读取到kill_list列表中 #这里改成你要解析的日志文件的名字
            ...
            while damage_time >= (get_time - 30) and num>start_num:
                ...
                while (int(re.findall(get_damage_time,kill_list[num1])[0]) <= (get_time+1)):
                        ...
                        if num1== len(kill_list):
                            break
        if  re.findall(find_death,li)!=[] and int(re.findall(find_death,li)[0][0])==get_time:#判断最后一击是否是来自于玩家
...
def k_tou_king(list):
    ...
    for i in range(0,10):
        ktou_num_list[i]=0  
    k_damage=list
    take_damage=re.compile(r'.*?"type":"DOTA_COMBATLOG_DAMAGE".*?')#其实这个可以跟上面的find_damge合并的，但是得改好些语句，我有点犯困了，不改了。
        ...
        if  re.findall(getkiller,line)!=[] and re.findall(flag,line)==[]:
            ...
            while int(kill_damage[i])==0 and i<10:
                if kill_damage[8]==0:#判断这次击杀是否为单杀
                    break
                if  kill_damage[i+1]==killer_damage[killer]:#排序后的伤害列表是这样的[0,0,0,0,0,2,3,4,5,6],[i+1]即为最小值,killer为击杀者的序号
                    ktou_num_list[killer]=ktou_num_list[killer]+1
                    break  
                i=i+1
                if killer==0:
                    print(ktou_num_list[0])
                    print(killer_damage)               
